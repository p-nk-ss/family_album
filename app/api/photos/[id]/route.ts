...
// Update a photo's note (the inscription shown on the flip-side in the viewer).
// Authored from the album editor. Same authority as deletion: the uploader or
// the creator of the album the photo lives in.
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const parsed = photoCaptionSchema.safeParse(await req.json().catch(() => null))
  if (!parsed.success)
  ...
  return NextResponse.json({ ok: true, caption: parsed.data.caption })
}
